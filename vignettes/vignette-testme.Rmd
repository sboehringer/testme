...
This package is meant to rapidly develop unit (or integration) tests for your personal library. The author maintains a set of R-scripts that he re-uses across projects. It is important that the meaning of functions does not change over time so that old analyses can be repeated even with the latest version of the scripts.
...
Unfortunately `R CMD check` behaves differently from `R CMD check --as-cran` yet behaving differently from `tools::testInstalledPackage`. `testme` therefore only installs the R testing mechanism when `R CMD check` is used. As long as as tests are installed when installing a package (`R CMD INSTALL --install-tests`), the tests be run using function `runPackageTests`.

# Advanced topics

## Using Expectations

One major optimization in the testing workflow of `testme` is that expectations are created automatically. Sometimes it is required or clearer to provide expectations explicitly. Expectations are deparsed R values. They are indicated by variables names `Ed` where `d` is an integer. The deparsing can be performed by function `Deparse` as provided by package `testme` or by the build-in `dput`.
...
Overall, a tight integration of several steps is achieved:
  ...
  * Reviewing tests thus collected emphasizes (or de-emphasizes) the importance of corner cases
