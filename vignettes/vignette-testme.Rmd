...
The first line initializes the testing environment. As we have not provided `testme` with what to expect, expectations are created in the first run. The `expectationsFolder` argument to `testmeEnvInit` specifies where to store the expectations. An output similar to the following is expected:
...
That's it. Running the tests will preserve the definition (meaning) of you function by storing the output of variables `Td`. Subsequent runs will check against these definitions. If your tests fail, you know that code relying the previous definitions might break.
